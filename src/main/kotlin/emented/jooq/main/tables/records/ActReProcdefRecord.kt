/*
 * This file is generated by jOOQ.
 */
package emented.jooq.main.tables.records

import emented.jooq.main.tables.ActReProcdef

import org.jooq.Record1
import org.jooq.impl.UpdatableRecordImpl

/**
 * This class is generated by jOOQ.
 */
@Suppress("UNCHECKED_CAST")
open class ActReProcdefRecord() : UpdatableRecordImpl<ActReProcdefRecord>(ActReProcdef.ACT_RE_PROCDEF) {

    open var id_: String?
        set(value): Unit = set(0, value)
        get(): String? = get(0) as String?

    open var rev_: Int?
        set(value): Unit = set(1, value)
        get(): Int? = get(1) as Int?

    open var category_: String?
        set(value): Unit = set(2, value)
        get(): String? = get(2) as String?

    open var name_: String?
        set(value): Unit = set(3, value)
        get(): String? = get(3) as String?

    open var key_: String?
        set(value): Unit = set(4, value)
        get(): String? = get(4) as String?

    open var version_: Int?
        set(value): Unit = set(5, value)
        get(): Int? = get(5) as Int?

    open var deploymentId_: String?
        set(value): Unit = set(6, value)
        get(): String? = get(6) as String?

    open var resourceName_: String?
        set(value): Unit = set(7, value)
        get(): String? = get(7) as String?

    open var dgrmResourceName_: String?
        set(value): Unit = set(8, value)
        get(): String? = get(8) as String?

    open var hasStartFormKey_: Boolean?
        set(value): Unit = set(9, value)
        get(): Boolean? = get(9) as Boolean?

    open var suspensionState_: Int?
        set(value): Unit = set(10, value)
        get(): Int? = get(10) as Int?

    open var tenantId_: String?
        set(value): Unit = set(11, value)
        get(): String? = get(11) as String?

    open var versionTag_: String?
        set(value): Unit = set(12, value)
        get(): String? = get(12) as String?

    open var historyTtl_: Int?
        set(value): Unit = set(13, value)
        get(): Int? = get(13) as Int?

    open var startable_: Boolean?
        set(value): Unit = set(14, value)
        get(): Boolean? = get(14) as Boolean?

    // -------------------------------------------------------------------------
    // Primary key information
    // -------------------------------------------------------------------------

    override fun key(): Record1<String?> = super.key() as Record1<String?>

    /**
     * Create a detached, initialised ActReProcdefRecord
     */
    constructor(
        id_: String? = null,
        rev_: Int? = null,
        category_: String? = null,
        name_: String? = null,
        key_: String? = null,
        version_: Int? = null,
        deploymentId_: String? = null,
        resourceName_: String? = null,
        dgrmResourceName_: String? = null,
        hasStartFormKey_: Boolean? = null,
        suspensionState_: Int? = null,
        tenantId_: String? = null,
        versionTag_: String? = null,
        historyTtl_: Int? = null,
        startable_: Boolean? = null
    ) : this() {
        this.id_ = id_
        this.rev_ = rev_
        this.category_ = category_
        this.name_ = name_
        this.key_ = key_
        this.version_ = version_
        this.deploymentId_ = deploymentId_
        this.resourceName_ = resourceName_
        this.dgrmResourceName_ = dgrmResourceName_
        this.hasStartFormKey_ = hasStartFormKey_
        this.suspensionState_ = suspensionState_
        this.tenantId_ = tenantId_
        this.versionTag_ = versionTag_
        this.historyTtl_ = historyTtl_
        this.startable_ = startable_
        resetChangedOnNotNull()
    }
}
